...
from cover_agent.test_template_generator import create_test_templates_if_needed
...
def parse_args_full_repo(settings: Dynaconf) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Specify the web framework (fastapi or flask). If not provided, will auto-detect."
    )
...
def find_test_files(args) -> list:
    ...
    # If no test files found, try to create template
    if not test_files:
        print("🔍 No test files found. Attempting to create default test template...")
        
        # Get user inputs
        source_file = getattr(args, 'source_file_path', None)
        user_framework = getattr(args, 'framework', None)
        
        if create_test_templates_if_needed(project_dir, language, user_framework, source_file):
            # Re-scan for test files after template creation
            print("🔄 Re-scanning for test files after template creation...")
            test_files = []
            for root, dirs, files in os.walk(project_dir):
                if not is_forbidden_directory(root.__add__(os.sep), language):
                    if "test" in root.split(os.sep):
                        ...
                        for file in files:
                            if "test" in file:
                                if filename_to_lang(file) == language:
                                    test_files.append(os.path.join(root, file))
            print(f"📊 Found {len(test_files)} test files after template creation.")
        else:
            print("❌ Failed to create test templates. Please create test files manually.")
